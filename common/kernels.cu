...
__global__ void calculate_C(size_t ndims,
  ...
  if ( tid < C_element_number ){
    ...
    // calculate C index for this tid
    size_t C_ind=0;
    for ( size_t dim=ndims-1; ; dim--){
      if (d_strides_C[dim] != 0){
        if ( tid / d_strides_C[dim] > 0 ){
    ...
    for ( size_t iter=0; iter < zero_cardinality_dim_tuples_C_element_number; ){
      ...
      size_t B_ind=0;
      size_t d_inds_F;
      size_t tmp_F_ind = F_ind;
      for ( size_t dim=ndims-1; ; dim--){

        if ( tmp_F_ind / d_strides_F[dim] > 0 ){
          d_inds_F = tmp_F_ind / d_strides_F[dim];
          tmp_F_ind -= d_inds_F*d_strides_F[dim];
        }else{
          d_inds_F = 0;
        }
