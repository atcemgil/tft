...
function val = get_element(tensor_data, cardinalities, index, VERBOSITY)
    strides=get_strides(cardinalities,VERBOSITY);
    cur_ind = sum(strides .* index);
    ind = cur_ind+1;
    ind_mod = -1; % for print statement
    if ind > numel(tensor_data) % otherwise must not add +1
        ind_mod = mod( ind , numel(tensor_data) ) + 1
        val = tensor_data( ind_mod );
    else
        val = tensor_data( ind );
    end
    ...
    if VERBOSITY > 2
        display(['get: ' ...
                ' ind ' num2str(ind) ' mod_ind ' num2str(ind_mod) ...
                ' val ' num2str(val) ...
                ]);
    end
end
