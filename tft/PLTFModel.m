...
classdef PLTFModel
    ...
    methods
        ...
        % name'. 
        % if disp == 'display' prints contraction operation
        % if disp == 'cmd' prints contraction operations as bsxfun+sum operations
        %[ 'contract START dim length ' num2str(length(dim))]
            ...
            if strcmp(disp, 'display')
                str = [char(tmp.name) ' = ' ];
                for i = 1:length(contracted_factor_inds)
                    str = [ char(str) ' ' char(newmodel.factors(contracted_factor_inds(i)).name) ];
                end
                fprintf( char(str) );
            end
        ...
        function [ocs_dims] = get_optimal_contraction_sequence_dims(obj, graph)
        ...
        function [] = print_optimal_contraction_sequence(obj)
            g = obj.schedule_dp();
            ocs_dims = obj.get_optimal_contraction_sequence_dims(g);
            newmodel = obj;
            for c_dim_ind = 1:length(ocs_dims)
                fprintf([ num2str(c_dim_ind) ' contract dimension ' char(ocs_dims{c_dim_ind}) ' -> ' ])
                newmodel = newmodel.contract( char(ocs_dims{c_dim_ind}), 'mem_analysis', '', 'display');
                fprintf( char(10) )
            end
        end
        ...
        function [commands] = get_ocs_commands(obj)
            g = obj.schedule_dp();
            ocs_dims = obj.get_optimal_contraction_sequence_dims(g);
            newmodel = obj;
            global commands;
        ...
        function [newmodel] = optimal_contact_all(obj)
            g = obj.schedule_dp();
            newmodel = obj;
            X_name = obj.get_factor_data_name( obj.observed_factor );
            newmodel = newmodel.contract_all('optimal', 'compute', X_name, g);
        end
       ...
       function [ordered_index_chars] = order_dims(obj, ...
        ...
        function [] = remote_gpu_pltf(obj, iternum, host_url, username, dataset, poll, force_upload)
        % executes remote job on given host with username
        % iternum: number of pltf iterations
        ...
        % data name is the key on remote data storage for experiment data
            ...
            display('Queueing job');
            url = [host_url '/queue_job?user=' username '&dataset=' dataset];
            res = urlread(url);
            if res == 'ok'
                display('ok')
            else
